...
import { ICredential, IResponseError } from '@/app/_common/types';
import { IContinueWithProviderRequest, ILoginRequest } from '@/app/auth/_interfaces';
  ...
  async jwt({ token, user, account }) {
    ...
    if (token?.userToken) {
      ...
      if (isAccessTokenExpire) {
        ...
        if (refreshTokenData?.statusCode === 200) token.userToken = refreshTokenData.data;
